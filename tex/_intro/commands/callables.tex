...
A \emph{callable} is a valid command with no space and control operators~\cite{control_operators}. The list of callables depends on the machine's state and is generally extended through \keyword{terraforming}. Some of the well-known callables are \emph{git}~\cite{git}, \emph{docker}~\cite{docker}, \emph{pushd}~\cite{pushd}, \emph{nano}~\cite{nano}. 
