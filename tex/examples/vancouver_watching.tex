...
\texttt{vanwatch}~\footurl{https://github.com/kamangir/vancouver-watching} is a \keywordd{callable}{callables} that,
%
\begin{enumerate}
...
\end{verbatim}
%
runs object detection~\footurl{https://hub.ultralytics.com/models/R6nMlK6kQjSsQ76MPqQM?tab=preview} on the images ingested~\ref{vanwatch_process} into \texttt{<object-name>} and updates \texttt{<area>.geojson}. \texttt{vanwatch process} reuses the inference in the object and completes the missing pieces. \texttt{+publish} \keywordd{tags}{tag} \texttt{object-name} for discovery by \texttt{update\_QGIS} and \keywordd{publishes}{publish} the object.
...
\end{verbatim}
%
\texttt{update\_QGIS} uses \keywordd{@git}{git} to maintain a copy of \texttt{geojson}s in the repo~\footurl{https://github.com/kamangir/vancouver-watching/tree/main/QGIS}.