...
\marginpar{Computational Model}
A group of operators create and maintain one or more repositories, using a system such as \keyword{git} and following a collective peer-reviewed \emph{pull-request}~\cite{pull_request} process. Each operator can access a set of \keyword{machines} and create \keyword{shells} on them to run \keyword{commands} to produce high value \keyword{objects}. Each \emph{operator} attempts to maximize an objective function that depends on the state of multiple machines.
