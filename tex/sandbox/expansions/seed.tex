\subsection{\texttt{@seed}}\label{seed}

It is often necessary to continue the operation on another \keyword{machine}. Examples include \texttt{ssh}ing to another machine, continuing the work on AWS SageMaker and between different images, running the code in a docker container or submitting it to AWS Batch, or running commands inside the Python Console in QGIS~\cite{QGIS_python_console}. The \texttt{@seed} expansion generates the code for the other machine to \keyword{terraform} and run a command or start accepting commands~\footnote{Does this concept exist in the literature?}.
%
\begin{verbatim}
@seed \
